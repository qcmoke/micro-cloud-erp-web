<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column prop="accountId" label="账户编号" min-width="150px" />
...
<script>
import { pageAccountApi, batchDeleteAccountApi } from '@/api/fms'
import Pagination from '@/components/Pagination'
import AccountEdit from './edit'

export default {
  name: 'AccountManage',
  components: { Pagination, AccountEdit },
  data() {
    return {
      query: {
        pageNum: 1,
  ...
  methods: {
    ...
    edit: function(row) {
      ...
      this.$refs.accountEdit.setRow({ ...row })
      this.dialog.isVisible = true
    },
    ...
    batchDelete: function() {
      ...
      this.$confirm(this.$t('tips.confirmDelete'), this.$t('common.tips'), {
        ...
        const accountIds = this.selection.map(row => row.accountId).join()
        this.loading = true
        batchDeleteAccountApi(accountIds)
          .then(r => {
            this.$message({
